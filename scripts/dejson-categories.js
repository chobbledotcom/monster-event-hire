#!/usr/bin/env bun
// One-shot: replace `json_ld:` literal frontmatter on categories with
// structured fields. The new _includes/wp-collectionpage-jsonld.html (shared
// with areas) rebuilds the JSON-LD at render time.
...
const dejsonOne = (path) => {
  ...
  const page = findFirst(graph, "CollectionPage");
...
const main = () => {
  ...
  const results = files.map((f) => dejsonOne(join(CATEGORIES, f)));
  const ok = results.filter((r) => r.ok);
  const skipped = results.filter((r) => r.skipped);
  console.log(`De-jsonified ${ok.length}/${files.length} categories`);
